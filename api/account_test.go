...
func TestListAccountsAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "BadRequest - Invalid PageId 0",
			query: listAccountsRequest{
				PageID:   0,
				PageSize: int32(n),
			},
		...
		{
			name: "BadRequest -  Invalid PageId -1",
			query: listAccountsRequest{
				PageID:   -1,
				PageSize: int32(n),
			},
		...
		{
			name: "BadRequest -  Invalid PageSize",
			query: listAccountsRequest{
				PageID:   1,
				PageSize: int32(4),
			},
	...
	for i := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			q := req.URL.Query()
			q.Add("pageId", fmt.Sprintf("%d", testCase.query.PageID))
			q.Add("pageSize", fmt.Sprintf("%d", testCase.query.PageSize))
